  ...
  try
  {
    ...
    if(records.length === size){
        ...
        if(testObj.isTestAlreadyAvailable === false){
          ...
          return res.json({ success: true, msg: `${records.length} questions available for test`,testId: testResponse?._id.toString() })
        }
        ...
        return res.json({ success: false, msg:"Cannot not find all the questions"})
    }
  }
  catch (error) {
